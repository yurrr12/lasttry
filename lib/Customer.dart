...
class Dbs {
  ...
  void createCustomer(Customer customer) async{
    print("Came: " + customer.toString());
    await Firebase.initializeApp();
    FirebaseFirestore.instance.collection('Customer').add({'id': customer.id, 'name': customer.name, 'password': customer.password, 'lastListened': customer.lastListened, 'liked': customer.liked, 'history': customer.history});
  }
  Future<Customer> loginCustomer(String name, String password) async {
    ...
    Stream<QuerySnapshot> stream = FirebaseFirestore.instance.collection(
        'Customer').snapshots();
    List<QuerySnapshot> list = await stream.toList();
    print("CON");
  }
